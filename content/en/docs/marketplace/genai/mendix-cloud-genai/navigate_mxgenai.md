...
To add data to your knowledge base, you can also ingest information stored in Confluence. Select the **Connect to Confluence** option and follow the steps to enter your connection details. Basic authentication is used to setup the connection, requiring only an API key without scopes, which you can create [here](https://id.atlassian.com/manage-profile/security/api-tokens). The datasource will show a badge **Initial Sync required** and once you've successfully created the new data source, you can initiate the data ingestion by clicking the **Sync Datasource** icon on the details card in the content overview. Please note: To reflect any Confluence updates in your knowledge base, you'll always need to trigger a synchronization. Additional buttons are available to view ingestion job details or delete the data source.
